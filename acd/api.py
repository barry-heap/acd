...
def export_tag(project: RSLogix5000Content, tag, output_path, owner: str = None) -> None:
    """Export a single tag as a standalone, partial L5X for Studio's native
    "Import Component" / Tags-folder Import feature.

    Same rationale as export_routine(): sidestep the save_acd()/FileInfo.Dat
    write-back limitation (Studio 5000 enforces the FileInfo.Dat container
    checksum on open -- see CLAUDE.md "ACD write-back") by handing Studio a
    partial L5X and letting *it* do the binary write + re-sign. Covers the
    common edit classes for a tag: create a new tag, change an existing
    tag's Description, or change its value -- edit the Tag object's fields
    (or construct a new Tag) before calling this.

    The wrapper mirrors export_routine()'s verified structure: only the
    container elements (<Controller>, <DataTypes>, <Tags>, and for a
    program-scope tag <Programs>/<Program>) carry Use="Context"; the single
    target <Tag> carries Use="Target"; no other element carries a Use=
    attribute (adding Use= to a non-target element was the confirmed trigger
    for a real Logix Designer import crash -- see export_routine()).

    *** CALIBRATION PENDING ***: unlike export_routine() (calibrated against a
    real Studio "Export Routine" output), the single-tag wrapper's exact
    TargetType/TargetSubType/ExportOptions and whether the target <Tag> is
    Use="Target" vs. plain have NOT yet been diffed against a real Studio
    "Export" of one tag. Do that before trusting an import (see CLAUDE.md).

    Args:
        project: The loaded project (from load_acd()) -- sources
            SchemaRevision/SoftwareRevision, the controller name, the tag's
            owning program (for program-scope tags), and any UDT the tag
            depends on (emitted under <DataTypes Use="Context">).
        tag: The Tag object to export. For a program-scope tag it must be
            one of project.controller.programs[i].tags[j]; for a
            controller-scope tag, one of project.controller.tags.
        output_path: Destination .L5X file path.
        owner: Optional "Owner" attribute (see export_routine()).

    Raises:
        ValueError: if `tag` isn't found in the controller or any program.
    """
    import datetime

    # Locate scope: controller-level, or which program owns it.
    owning_program = None
    if not any(t is tag for t in project.controller.tags):
        for p in project.controller.programs:
            if any(t is tag for t in p.tags):
                owning_program = p
                break
        else:
            raise ValueError(
                "tag not found in controller tags or any program's tags -- pass "
                "the same Tag object obtained from project.controller.tags[i] or "
                "project.controller.programs[i].tags[j]"
            )
    ...
    # UDT dependency: if the tag's DataType is a project-defined UDT, include
    # its definition under <DataTypes Use="Context"> (same as export_routine).
    referenced_type_names = {tag.data_type.upper()} if getattr(tag, "data_type", None) else set()
    referenced_data_types = [
        dt for dt in project.controller.data_types
        if dt.name.upper() in referenced_type_names
    ]
    data_types_xml = "".join(dt.to_xml() for dt in referenced_data_types)

    target_tag_xml = _inject_use_attr(tag.to_xml(), "Tag", "Target")
    owner_attr = f' Owner="{_escape_xml_attr(owner)}"' if owner else ""
    ...
    if owning_program is None:
        ...
        # Program-scope tag: nest under <Programs>/<Program>.
        body = (
            f'<DataTypes Use="Context">\n{data_types_xml}\n</DataTypes>\n'
            f'<Tags Use="Context">\n</Tags>\n'
            f'<Programs Use="Context">\n'
            f'<Program Use="Context" Name="{_escape_xml_attr(owning_program.name)}">\n'
            f'<Tags Use="Context">\n{target_tag_xml}\n</Tags>\n'
            f'</Program>\n'
            f'</Programs>\n'
        )
    ...
    xml = (
        ...
        f'<RSLogix5000Content SchemaRevision="{project.schema_revision}" '
        f'SoftwareRevision="{project.software_revision}" '
        f'TargetName="{_escape_xml_attr(tag.name)}" '
        f'TargetType="Tag" '
        f'{owner_attr}ContainsContext="true" ExportDate="{export_date}" '
        f'ExportOptions="{export_options}">\n'
        f'<Controller Use="Context" Name="{_escape_xml_attr(controller_name)}">\n'
